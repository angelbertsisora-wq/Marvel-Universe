      ...
      >
        ...
        <div className="relative h-[60%] w-full overflow-hidden">
          ...
          <button
            ...
            } ${isTogglingFavorite ? 'opacity-50 cursor-wait' : 'cursor-pointer'} ${favoriteError ? 'ring-2 ring-red-500' : ''}`}
            title={
              isTogglingFavorite 
                ? 'Updating...' 
                : isAuthenticated 
                  ? (isFavorite(film.id) ? 'Remove from favorites' : 'Add to favorites') 
          ...
          {/* Error Prompt */}
          {favoriteError && (
            <div className="absolute bottom-20 right-4 z-30 animate-fadeIn rounded-lg bg-red-500/95 px-4 py-2 backdrop-blur-sm">
              <p className="font-robert-medium text-xs text-white">
                {favoriteError}
              </p>
            </div>
          )}
...
const UpcomingFilms = () => {
  ...
  // Get films data from Inertia props (fetched by Laravel backend)
  const incomingFilmsData = props.upcomingFilms;
  const [filmsData, setFilmsData] = useState(incomingFilmsData);
  const [isLoading, setIsLoading] = useState(!incomingFilmsData);
  const [error, setError] = useState(incomingFilmsData ? null : 'Failed to load films data');

  // Enrich data with video URLs when it's available
  useEffect(() => {
    if (incomingFilmsData) {
      const enrichedData = {
        ...incomingFilmsData,
        video_url: filmVideoUrls[incomingFilmsData.id] || null,
        following_production: incomingFilmsData.following_production ? {
          ...incomingFilmsData.following_production,
          video_url: filmVideoUrls[incomingFilmsData.following_production.id] || null,
        } : null,
      };
      setFilmsData(enrichedData);
      setIsLoading(false);
      setError(null);
    } else {
      // If no data from props, try to fetch from backend API endpoint
      const fetchFromBackend = async () => {
        try {
          setIsLoading(true);
          setError(null);
          console.log('🔄 Fetching films from backend API...');
          
          const response = await fetch('/api/marvel/upcoming-films', {
            headers: {
          ...
          if (response.ok) {
            ...
            console.log('✅ Backend API data received:', data);
            ...
            const enrichedData = {
              ...data,
              video_url: filmVideoUrls[data.id] || null,
              following_production: data.following_production ? {
                ...data.following_production,
                video_url: filmVideoUrls[data.following_production.id] || null,
              } : null,
            };
            setFilmsData(enrichedData);
            setIsLoading(false);
  ...
  const films = filmsData ? [
    { ...filmsData, isNext: true },
    ...(filmsData.following_production ? [{ ...filmsData.following_production, isNext: false }] : []),
  ] : [];
    ...
    >
      ...
      <div className="container relative z-10 mx-auto px-5 md:px-10">
        ...
        {isLoading && (
          <div className="flex min-h-[400px] items-center justify-center">
            <div className="text-center">
              ...
              <p className="font-circular-web text-blue-50/70">Loading upcoming films...</p>
            </div>
          </div>
        )}
        ...
        {error && !isLoading && (
          <div className="flex min-h-[400px] items-center justify-center">
            <div className="text-center">
              <p className="mb-4 font-circular-web text-lg text-red-300">Failed to load films data</p>
              <p className="font-circular-web text-sm text-blue-50/70">{error}</p>
              <button
                onClick={() => window.location.reload()}
                className="mt-4 rounded-lg bg-violet-400 px-6 py-2 text-sm font-semibold text-black transition hover:bg-violet-300"
              >
                Retry
              </button>
            </div>
        ...
        {!isLoading && !error && filmsData && (
          <>
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6">
                    ...
                    <div className="mt-3 inline-flex items-center gap-3 rounded-full bg-violet-300/90 px-4 py-2 text-black backdrop-blur">
                      ...
                      <span className="font-circular-web text-sm">{new Date(filmsData.release_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                    <div className="mt-4">
                      <LiveCountdown releaseDate={filmsData.release_date} isCompact />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {films.length > 0 ? (
              ...
              <div className="flex min-h-[400px] items-center justify-center">
                <p className="font-circular-web text-blue-50/70">No upcoming films available.</p>
              </div>
            )}
            ...
            {filmsData && (
              <div className="mt-20 text-center">
                <div className="mx-auto inline-block rounded-lg border border-white/20 bg-black/50 px-8 py-6 backdrop-blur-sm">
                  ...
                  <LiveCountdown releaseDate={filmsData.release_date} />
                </div>
              </div>
            )}
          </>
